...
exports.updateArticle = (value)=>{
    ...
    let _sql = `UPDATE blog_article SET title='😄',description='${description}', contents='${contents}', updateTime='${updateTime}'  WHERE id=${id}`
    return query(_sql,value)
}
...
exports.getUserById = (value) => {
    let _sql = `select * from users where id="${value}"`
    return query(_sql, value)
}
// 发表评论
