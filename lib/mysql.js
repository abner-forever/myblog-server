...
exports.addArticle = ({ title, description, content, userId }) => {
    ...
    let _sql = `INSERT INTO article (title, author, userId, description, content, type ,viewCount, likeCount, hot, createTime, updateTime) VALUES ( '${title}', NULL, ${userId}, '${_description}', '${_content}', NULL, NULL,  NULL, NULL, NOW(), NOW());`
...
exports.updateArticle = (value) => {
    ...
    const _content = strtoBase64(content);
    const _description = strtoBase64(description);
    let _sql = `UPDATE article SET title='${title}', description='${_description}', content='${_content}', updateTime=NOW() WHERE id=${id}`
...
exports.getUserById = (userId) => {
    let _sql = `select userId, user_name as userName, avator, sex, phone from user where userId=?`
