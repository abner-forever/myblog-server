...
exports.addArticle = ({ title, description, content, userId }) => {
    ...
    let _sql = `INSERT INTO article (
        ...
        create_time,
        update_time
        ) VALUES ( '${title}', ${userId}, '${_description}', '${_content}', NULL, '${0}',  NULL, NULL, NOW(), NOW());`
...
exports.updateArticleViewCount = (id,count)=>{
    let _sql = `UPDATE article SET view_count = ${count} WHERE id=${id}`
...
exports.getComments = (id) => {
    let _sql = `select avator,content,user_id as userId, id, user_name as userName, moment from article_comment where postid=${id} order by moment desc`
    return query(_sql, id)
}
...
exports.addComment = (value) => {
    ...
    const _sql = `INSERT INTO article_comment (user_id, content, moment, postid, avator,user_name, create_time)
     VALUES ('${userId}', '${content}', '${moment}', '${postid}', '${avator}', '${userName}', NOW());`
    return query(_sql, value)
}
//删除文章评论
...
exports.register = (value) => {
    const { userName: name, password, sex, avator } = value
    let _sql = `INSERT INTO user (sex, name, password, avator, register_time, phone) VALUES (${sex}, '${name}', '${password}', '${avator}', NOW(), NULL);`;
    return query(_sql, value)
}
//检查用户名是否存在
